...
import getDateString from '../lib/get-date-string.js';
 ...
 */
const getTargetPath = function getTargetPath (destDir, filepath, options) {
  options = options || {};
  ...
  if (options.appendHash) {
    ...
    hash.update(`${destDir}${filepath}${namePart}${Math.random()}`);
